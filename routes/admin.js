...
router.get('/users/new',
  ...
  async (req, res) => {
    ...
    res.render('users/form', {
      title: 'Tạo User mới',
      user: req.user,
      userData: {}, roles, agencies, errors: null
    });
  }
);
...
router.post('/users/new',
  ...
  async (req, res) => {
    try {
      const { username, password, roleId, agencyId } = req.body;
      await User.create({ username, password, role: roleId, agency: agencyId });
...
// List
router.get('/agencies',
  auth, rbac('agencies:read'),
  async (req, res) => {
    const agencies = await Agency.find();
    res.render('agencies/list', { title: 'Agencies', agencies, user: req.user });
  }
);

// Create form
router.get('/agencies/new',
  auth, rbac('agencies:create'),
  (req, res) => {
...
// Edit form
router.get('/agencies/:id/edit',
  auth, rbac('agencies:update'),
  async (req, res) => {
...
// Delete
router.post('/agencies/:id/delete',
  auth, rbac('agencies:delete'),
  async (req, res) => {
...
// List
router.get('/branches',
  auth, rbac('branches:read'),
  async (req, res) => {
    const branches = await Branch.find().populate('agency');
    res.render('branches/list', { title: 'Branches', branches, user: req.user });
  }
);

// Create form
router.get('/branches/new',
  auth, rbac('branches:create'),
  async (req, res) => {
    const agencies = await Agency.find();
...
// Edit form
router.get('/branches/:id/edit',
  auth, rbac('branches:update'),
  async (req, res) => {
...
// Delete
router.post('/branches/:id/delete',
  auth, rbac('branches:delete'),
  async (req, res) => {
...
router.get('/keys',
  auth, rbac('keys:read'),
  async (req, res) => {
    const keys = await Key.find().populate({ path: 'branch', populate: 'agency' });
