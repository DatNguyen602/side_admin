...
router.get("/mono/:id", auth, async (req, res) => {
    try {
        ...
        if (!message) {
            return res
                .status(404)
                .json({ message: "Không tìm thấy tin nhắn!" });
        }
        res.json(message);
    } catch (error) {
...
router.post("/new", auth, async (req, res) => {
    try {
        ...
        if (!mesNew)
            return res.status(204).json({ message: "No new messages" });
        ...
        messages = await Promise.all(
            messages.map(async (msg) => {
                ...
                for (const r of msg.reactions) {
                    if (!groupedReactions[r.type])
                        groupedReactions[r.type] = [];
...
router.delete("/react/all", auth, async (req, res) => {
    ...
    try {
        ...
        await message.save();
        res.status(200).json({
            message: "All user reactions deleted.",
            reactions: message.reactions,
        });
    } catch (err) {
