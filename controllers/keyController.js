...
exports.list = (req, res) =>
  Key.find().populate({ path: 'branch', populate: 'agency' }).then(keys => res.json(keys));
...
exports.loginAndCreateSession = async (req, res) => {
  ...
  // 3. Tìm key theo token
  const key = await Key.findOne({ token });
  if (!key) return res.status(404).json({ message: "Key không tồn tại" });
  if (key.status === 'revoked') return res.status(403).json({ message: "Key đã bị thu hồi" });
  if (key.status === 'used') return res.status(500).json({ message: "Key đang được sử dụng!" });

  // 4. Cập nhật trạng thái key
  if (key.status === 'issued') {
    key.status = 'used';
    await key.save();
  }

  // 5. Truy ngược thông tin branch & agency
...
exports.createSession = async (req, res) => {
  ...
  // Cập nhật trạng thái key nếu cần
  if (key.status === 'issued') {
    key.status = 'used';
    await key.save();
  }

  // Truy ngược lên thông tin branch
  const branch = await Branch.findById(key.branch);

  // Truy ngược lên thông tin agency từ branch
  const agency = await Agency.findById(branch.agency);

  // Tạo phiên làm việc
  const session = await Session.create({ user: userId, key: key });
  res.json({ message: "Phiên làm việc đã được tạo", session, branch: branch, agency: agency });
};
...
exports.cancelSession = async (req, res) => {
  ...
  if (!session) return res.status(404).json({ message: "Phiên làm việc không tồn tại hoặc đã hủy" });
...
exports.loginAndCancelSession = async (req, res) => {
  ...
  if (!session) return res.status(404).json({ message: "Không tìm thấy phiên làm việc đang hoạt động" });
